...
def serve() -> None:
    """Start the FastAPI server (registered as `dc-server` script)."""
    try:
        try:
            while True:
...
def init() -> None:
    """Scaffold config at ~/.config/deep-claude/.env (registered as `dc-init`)."""
    config_dir = Path.home() / ".config" / "deep-claude"
    ...
    print("Edit it to set your API keys and model preferences, then run: dc-server")
...
def _launch_claude_with_brand(brand: str, argv: Sequence[str] | None = None) -> None:
    """Launch Claude Code with Deep Claude Code proxy and a branded terminal title."""

    # Set terminal window title
    print(f"\033]0;{brand}\007", end="", file=sys.stderr, flush=True)

    settings = get_settings()
    proxy_root_url = local_proxy_root_url(settings)
    if error := _preflight_proxy(proxy_root_url):
        print(
            f"{brand} proxy is not reachable at {proxy_root_url}: {error}",
            file=sys.stderr,
        )
        print("Start it in another terminal with: dc-server", file=sys.stderr)
        raise SystemExit(1)
    ...
    env["_DCC_BRAND"] = brand
    if "--require" not in node_opts:
        env["NODE_OPTIONS"] = f"{node_opts} --require {patch_js}".strip()
...
def launch_dcc(argv: Sequence[str] | None = None) -> None:
    """Launch Claude Code branded as Deep Claude Code (dcc)."""
    _launch_claude_with_brand("Deep Claude Code", argv)


def launch_dc_claude(argv: Sequence[str] | None = None) -> None:
    """Launch Claude Code branded as Deep Claude Code (dc-claude)."""
    _launch_claude_with_brand("Deep Claude Code", argv)


# Keep original entry point for backwards compatibility
