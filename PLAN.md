...
`deep-claude` is an Anthropic-compatible proxy with optional messaging
workers:
...
Product smoke lives under `smoke/` and is enabled with `DCC_LIVE_SMOKE=1`.
...
explicitly selected by `DCC_SMOKE_PROVIDER_MATRIX`, missing configuration should
