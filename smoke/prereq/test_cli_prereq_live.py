...
from smoke.lib.child_process import cmd_dc_init, cmd_deep_claude_serve
...
def test_dc_init_scaffolds_user_config(
    ...
    assert (tmp_path / ".config" / "deep-claude" / ".env").is_file()


def test_deep_claude_entrypoint_starts_server(smoke_config: SmokeConfig) -> None:
    with start_server(
        smoke_config,
        command=cmd_deep_claude_serve(),
...
def test_claude_cli_prompt_when_available(
    ...
    with start_server(
        ...
        result = subprocess.run(
            [claude_bin, "-p", "Reply with exactly DCC_SMOKE_PONG"],
            cwd=tmp_path,
            env=env,
            capture_output=True,
            text=True,
    ...
    if "DCC_SMOKE_PONG" not in result.stdout:
