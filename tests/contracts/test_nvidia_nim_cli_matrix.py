...
def test_nvidia_nim_cli_matrix_report_shape_and_redaction(
    ...
    run = ClaudeCliRun(
        ...
        returncode=0,
        stdout="DCC_NIM_BASIC secret-nim-key",
        stderr="",
        duration_s=1.25,
    )
...
def test_openrouter_free_cli_matrix_report_shape_and_redaction(
    ...
    run = ClaudeCliRun(
        command=("claude", "-p", "redacted"),
        returncode=0,
        stdout="DCC_OPENROUTER_FREE_BASIC secret-openrouter-key",
        stderr="",
        duration_s=1.25,
    )
    outcome = make_outcome(
        ...
        feature="basic_text",
        marker="DCC_OPENROUTER_FREE_BASIC",
        run=run,
...
def test_cli_matrix_missing_agent_catalog_is_harness_bug(tmp_path: Path) -> None:
    ...
    outcome = make_outcome(
        ...
        source="openrouter_free_cli_default",
        feature="subagent_task",
        marker="DCC_OPENROUTER_FREE_TASK",
        run=run,
        log_delta=(
...
def test_nvidia_nim_cli_timeout_is_not_model_missing(
    ...
    run = ClaudeCliRun(
        command=("claude", "-p", "x"),
        returncode=None,
        stdout='{"type":"assistant","content":[{"type":"text","text":"DCC_NIM_TOOL"}]}',
        stderr="",
        duration_s=45.0,
        timed_out=True,
    )
