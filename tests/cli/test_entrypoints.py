"""Tests for cli/entrypoints.py — dc-init scaffolding logic."""
...
def _run_init(tmp_home: Path) -> tuple[str, Path]:
    ...
    env_file = tmp_home / ".config" / "deep-claude" / ".env"
    printed: list[str] = []
...
def test_init_creates_parent_directories(tmp_path: Path) -> None:
    """init() creates ~/.config/deep-claude/ even if it doesn't exist."""
    config_dir = tmp_path / ".config" / "deep-claude"
...
def test_init_prints_next_step_hint(tmp_path: Path) -> None:
    """init() tells the user to run dc-server after editing .env."""
    output, _ = _run_init(tmp_path)

    assert "dc-server" in output
...
def test_cli_scripts_are_registered() -> None:
    ...
    assert scripts["dc-server"] == "cli.entrypoints:serve"
    assert scripts["deep-claude"] == "cli.entrypoints:serve"
    assert scripts["dc-claude"] == "cli.entrypoints:launch_claude"
...
def test_launch_claude_unreachable_proxy_exits_with_hint(
    ...
    assert "dc-server" in captured.err
